...
预设里的 `ctx` 只是**起点**。侧栏另有一个上下文滑块,可以按 token 自由调,
并且**按模型和 KV 精度分别标出「实测安全区」**。

### 实测的操作包线(本机 RTX 5060 Laptop 8GB)

两个 Bonsai 都是 `qwen35` **混合注意力**架构 —— `block_count=64`,但
`full_attention_interval=4`,**只有 16 层带 KV cache**,其余 48 层是线性注意力
(状态恒定)。所以上下文比同尺寸普通模型便宜得多:实测 KV 斜率约 23 MiB/1K token,
吃显存的从来是**权重**,不是上下文。

| 模型 | 权重 | KV 精度 | 满速上限 | 显存 | 速度 |
|---|---|---|---|---|---|
| Bonsai 27B 1-bit | 3.54 GB | q4_0 | **192K** | 7809 MiB | 41 tok/s |
| Bonsai 27B 1-bit | 3.54 GB | q8_0 | 96K | 7583 MiB | 41 tok/s |
| 三元版 | 5.54 GB | q4_0 | **96K** | 7869 MiB | 30–32 tok/s |
| 三元版 | 5.54 GB | q8_0 | 48K | 7670 MiB | 30–32 tok/s |

**提高 KV 精度 = 上下文减半。** 而实测两边输出质量看不出差异
(25 次生成,最长同字符连续都在 4–16,无乱码无塌缩),所以默认 q4_0。

### 视觉几乎不占显存

`--no-mmproj-offload` 把 0.59 GiB 的投影器留在**系统内存**里,所以开视觉与不开
差 ≈0 MiB(64K:7274 vs 7319;96K:7888 vs 7869)。另外 1-bit 那个模型的 GGUF 里
**没有 `qwen35.vision.*` 键**,它根本不支持传图 —— 视觉只属于三元版。

### ⚠ 溢出是静默的,而且显存读数看不出来

`-ngl 99` 是用户显式指定的,llama.cpp 于是**跳过自动显存适配**,只在日志留一行:
...
然后把 KV cache 挪到主机内存、注意力改由 CPU 算 —— **不报错、不退出,只是慢 6-8 倍**。
...
两行一模一样。所以界面上那条红线只是**静态**提示,真正的运行时判据是
**每秒生成多少 token** —— 见下面的启动探针。

### 启动探针

服务就绪后,外壳在后台跑一次固定长度(24 token)的生成,拿
`timings.predicted_per_second` 与该模型的实测基线比:

- 基线:1-bit ≈ 41 tok/s,三元版 ≈ 31 tok/s
- 低于基线的 **60%** 判定为溢出

实测验证(`node tools/test_probe.mjs`,同一个模型同一上下文,只换 KV):

```
三元版 @64K + q4_0 -> 32.2 tok/s,比值 1.04,判定满速   ✅
三元版 @64K + q8_0 ->  6.1 tok/s,比值 0.20,判定溢出   ✅
```

阈值落在 0.20 与 1.04 之间的巨大空档里,不敏感。整个探测约 9–15 秒。

**探针要跑两次**:模型加载后的第一次推理要建 CUDA 图,直接测会得到 2-5 tok/s
的假结果 —— 见下面「首次对话慢是正常的」那一节,写这个探针的第一版就栽在这里。
第一次只当预热丢弃。
...
- 本机完整数据、复现脚本、以及"第一次测量为什么是错的",见
  [`docs/上下文能力实测报告.md`](docs/上下文能力实测报告.md);
  产生那些数字的脚本在 [`docs/measure/`](docs/measure/)
- 三元版**必须**用 prism 构建(PTQ1_0 是 ggml type 143,上游构建直接拒绝加载)
- 同进程内比较多个配置,**必须先预热** —— `#1: 32.5s / #2: 1.3s / #3: 0.8s`
