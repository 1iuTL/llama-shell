...
塌缩只出现在**思考路径**上,所以 `--reasoning off` 能让 PTQ1_0 重新可用:

| 配置(Heretic 版,问 `1+1等于几`) | 结果 |
|---|---|
| 默认(思考开) | reasoning 507 个连续斜杠,**content 为空,答不出** |
| `--reasoning off` | content = `1+1等于2。` **答对了** |

要注意两点:

1. **答案会先出来,退化发生在之后。** 有一次输出是 `1+1 等于 **2**。` 紧跟 517 个斜杠 —— 答案是对的,只是模型不肯停,继续吐废字符直到上限。所以"能读"但浪费 token 和时间。
2. **不是 100% 稳定。** 2 次里 1 次干净、1 次拖尾巴。当兜底手段可以,不能当可靠方案。

想让这类模型可用,把侧栏的思考档位设成「关闭思考」。默认的 Q1_0 则不需要这个妥协。
