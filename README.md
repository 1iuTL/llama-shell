...
### 附:加载慢的那 19 秒是官方构建的固有开销(2026-09-25 实测)

官方构建启动时要在加载阶段额外做准备。同一台机器、同一模型、同一套参数实测:

| 构建 | 「threadpool init → 权重就位」 | 总加载 |
|---|---|---|
| PrismML 官方(`llama-prism`) | 19.2 s | ~24 s |
| 社区 sudoingX(`llama-cpp-mmq`) | **2.5 s** | **~7 s** |

后者看起来是"免费加速",但它在**推理时**会塌缩(见上一节的实测表),所以这 19 秒没有捷径:

- 官方构建的全部命令行开关里,没有任何 Hadamard / 预计算相关的加速选项;
- `--no-mmap` 与 `--load-mode` 各档都试过,对这一段没有影响;
- 加载期间 GPU 全程空闲(P8/285MHz),PCIe 链路还会自己降到 Gen1 —— 这些是
  「CPU 在准备、GPU 在等」的**现象**,不是硬件故障。当时按硬件方向查了很久才回到这里。

顺带两条容易误判的观察:

- `nvidia-smi` 报 `Gen4 x8`(卡本身支持 Gen5 x16)是本机长期状态,**与加载变慢无关**,
  不要当成链路故障去修。
- 系统日志里偶发的 `WHEA-Logger` 事件 17(PCI Express Root Port / AER)是**已纠正**错误,
  关掉 PCIe ASPM 后不再新增,同样与加载耗时无关。

实用建议:模型**加载一次后别频繁停/启**,那 19 秒只在启动时付一次。
### 试过但**无效**的手段
